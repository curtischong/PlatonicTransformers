...
def minimum_image_displacement(
    ...
    """Wrap Cartesian displacements into the closest periodic image.

    Args:
        displacement: Cartesian vectors with shape ``[..., D]``.
        lattice: Row-vector cell matrices broadcastable to ``[..., D, D]``.
        pbc: Boolean periodic flags broadcastable to ``[..., D]``.

    Returns:
        Cartesian displacements after applying the minimum-image convention.
    """
    if lattice is None:
        return displacement

    spatial_dim = displacement.shape[-1]
    if lattice.shape[-2:] != (spatial_dim, spatial_dim):
        raise ValueError(
            f"lattice last dimensions must be ({spatial_dim}, {spatial_dim}), "
            f"got {tuple(lattice.shape[-2:])}"
        )

    frac = torch.linalg.solve(
        lattice.transpose(-1, -2),
        displacement.unsqueeze(-1),
    ).squeeze(-1)

    if pbc is None:
        pbc = torch.ones(spatial_dim, dtype=torch.bool, device=displacement.device)
    pbc = pbc.to(device=displacement.device, dtype=torch.bool)
    frac = torch.where(pbc, frac - frac.round(), frac)

    return torch.matmul(frac.unsqueeze(-2), lattice).squeeze(-2)
