#!/bin/bash
#SBATCH --job-name=PT2-sig4-ema-dyn-ivi
...
# Long PT-2 dynamic-batching run on ivi-cluster: its7gzf1 recipe + sigma=4.0 + EMA + wd=1e-8.
# - Identical to its7gzf1 (run_pt2_upstream_long_dynamic.sh) except:
#     rope_sigma 1.5 -> 4.0
#     weight_decay 0 -> 1e-8
#     EMA enabled
# - EMA decay passed as $1.
#
# Partition + GPU type are NOT pinned here — pass them via sbatch CLI:
#   sbatch -p all6000 --account=all6000users --gres=gpu:rtx_6000:1 \
#          --job-name=PT2-sig4-ema9999-dyn-ivi-all6000 \
#          scripts/run_pt2_upstream_long_sig4_ema_dyn_ivi.sh 0.9999
#   sbatch -p geodude --account=geodudeusers --gres=gpu:rtx_a5000:1 \
#          --job-name=PT2-sig4-ema999-dyn-ivi-geodude \
#          scripts/run_pt2_upstream_long_sig4_ema_dyn_ivi.sh 0.999

set -e
mkdir -p /home/ebekker/platonic-omol/logs

EMA_DECAY=${1:?"Usage: sbatch ... $0 <ema_decay e.g. 0.9999>"}
...
EXP_NAME="pt2-upstream-add-sig4-ema${EMA_TAG}-dyn-ivi-${SLURM_JOB_PARTITION}"

echo "=== Long run (dyn-batching, sigma=4, EMA=${EMA_DECAY}, wd=1e-8): ${EXP_NAME} ==="
echo "Date:  $(date)"
...
COMMON_OVERRIDES=(
    ...
    wandb.group=pt2-upstream-sig4-ema-ivi
    seed=1
)
