#!/bin/bash
# OMol 4M: rope_sigma=2.0, 2-GPU DDP test (per-rank bs=64 -> effective 128)
# 80 epochs, 5-day wall time, val every 2500 steps, limit_val_batches=250
# (val cadence halved and subset halved per-rank so the combined 32K-sample
# val set and 320K-sample x-axis cadence match the 1-GPU sig=2.0 run.)
#
# sbatch --job-name=omol-4m-sig2-2gpu --account=gusei11738 --partition=gpu_h100 \
#        --gres=gpu:h100:2 --ntasks=1 --cpus-per-task=32 --mem=300G \
#        --time=5-00:00:00 --output=slurm-omol-4m-sig2-2gpu-%j.out \
#        scripts/run_4m_sig2_2gpu_snellius.sh
#
# Note: single SLURM task, Lightning DDP subprocess-spawns rank 1 itself.
# SLURM_JOB_NAME=bash disables Lightning's SLURM auto-detect so it uses its
# subprocess launcher instead of expecting srun to have launched all ranks.
...
python train_omol.py \
    ...
    +trainer.limit_val_batches=250 \
    exp_name=omol-4m-sig2-2gpu \
