...
# Hipster (RTX 6000 Ada sm_89, 48GB) counterpart of run_pt2_long_sig4_wd1e4_bf16.sh.
# Same recipe, same `+precision=bf16_h100` preset (TF32 + bf16-mixed + compile w/
# dynamic=true + auto-applied dynamo knobs). The preset's contents are not actually
# H100-specific: TF32 + bf16 mixed precision + dynamic compile work fine on Ada.
# Only hardware-specific change: attention_backend=scatter (we don't ship a flash-
# attn build for sm_89 on this venv).
#
# Companion to run_pt2_long_sig4_wd1e4_fp32_hipster.sh — both submit to the same
# W&B group `pt2-sig4-wd1e4-precision-comparison-hipster` for overlay.
#
# Usage:
#   sbatch --job-name=PT2-sig4-wd1e4-bf16-hipster scripts/run_pt2_long_sig4_wd1e4_bf16_hipster.sh
...
EXP_NAME="pt2-sig4-wd1e4-bf16-hipster${EXP_SUFFIX}"

echo "=== Long run hipster (sig4, wd1e4, dyn-batch, bf16_h100 preset): ${EXP_NAME} ==="
echo "Date:  $(date)"
