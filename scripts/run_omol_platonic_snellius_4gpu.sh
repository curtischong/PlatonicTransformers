#!/bin/bash
# Snellius 4× H100 DDP production run. Mirrors qcczbpfn's effective batch
# (4 × 3000 atoms / 600k edges per rank = 12000 atoms / 2.4M edges per
# optimizer step) on H100 hardware. This is the cleanest snellius-side
# apples-to-apples test against qcczbpfn (4× RTX 6000 Ada, same recipe).
#
# Same recipe as run_omol_platonic_snellius_4gpu_1h.sh, just with a long
# wall time so the 20-epoch run can complete.
...
#SBATCH --job-name=PR-omol-4gpu-snellius
