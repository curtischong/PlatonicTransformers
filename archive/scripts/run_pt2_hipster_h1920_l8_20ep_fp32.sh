...
# Hipster mirror of scripts/run_pt2_h1920_l8_20ep_fp32.sh (Snellius canonical).
# Same env-var interface and overrides; only changes are hipster plumbing:
# venv path, CUDA 12.3 pin, DATA_PATH=/scratch/ebekker/omol, ulimit fix.
#
# Canonical submit to reproduce yxh5y61s exactly (Snellius 22783311) on
# hipster performance (rtx_6000_ada/sm_89) with flash:
#   sbatch --job-name=PT2-hipster-qkn-sg-uk-flash \
#     --export=ALL,MAX_ATOMS=12000,WD=1e-8,LAYER_SCALE=null,FFN_FACTOR=2,\
...
#     scripts/run_pt2_hipster_h1920_l8_20ep_fp32.sh

set -e
mkdir -p /home/ebekker/platonic-omol/logs

# Hipster default soft ulimit -n is 1024 — too low for DataLoader workers +
# dynamo's pipe-based AOT-repro profiler. Without this, both bf16 and fp32 die
# in the first 30s with "Pin memory thread exited unexpectedly" / "pipe()
# failed". Raise to the hard cap.
...
# hipster has 11.4 + 12.3 + 12.9; pin to 12.3 (cu123 is ABI-compatible with cu128 torch).
...
EXP_NAME="pt2-hipster-h${HIDDEN_DIM}-l${NUM_LAYERS}-${LS_TAG}${FFN_TAG}${ACT_TAG}${RACT_TAG}${OPT_TAG}${RS_TAG}${ROV_TAG}${BACKEND_TAG}${QK_TAG}${SG_TAG}${UK_TAG}${NT_TAG}${CSM_TAG}${CSL_TAG}${CSF_TAG}${CSI_TAG}${EW_TAG}${FW_TAG}${R_TAG}${LG_TAG}${EMA_TAG}${LR_TAG}${BS_TAG}${SOLID_TAG}-wd${WD_TAG}-20ep-n${MAX_ATOMS}"

echo "=== PT-2 hipster 20ep (max_atoms=${MAX_ATOMS}, backend=${ATTENTION_BACKEND}): ${EXP_NAME} ==="
echo "Date:  $(date)"
...
OVERRIDES=(
    ...
    wandb.group=pt2-vs-esen-sm-direct-n${MAX_ATOMS}-hipster
    seed=1
)
