...
# 1-hour smoke: bf16-mixed + dynamo-tuned compile.
# Companion to run_pt2_upstream_smoke_sig4_wd1e4_dyn_compile_fp32.sh. Same recipe + knobs,
# only delta is +trainer.precision=bf16-mixed and PSL_FAST_BACKENDS=1 (TF32 + cudnn.benchmark).
#
# Usage:
#   sbatch --job-name=PT2-sig4-wd1e4-dyn-compile-bf16 scripts/run_pt2_upstream_smoke_sig4_wd1e4_dyn_compile_bf16.sh
...
EXP_NAME="pt2-upstream-add-sig4-wd1e4-dyn-compile-bf16"

echo "=== SMOKE (1h, dyn-batching, sigma=4, wd=1e-4, bf16-mixed + dynamo-tuned compile): ${EXP_NAME} ==="
echo "Date:  $(date)"
...
echo "       trainer.precision=bf16-mixed, TORCH_LOGS=${TORCH_LOGS}"
echo "Branch + commit:"
