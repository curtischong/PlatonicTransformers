...
# Hipster + flash-attn variant of fp32. Same recipe as run_pt2_long_sig4_wd1e4_fp32_hipster.sh
# but attention_backend=flash. Note: flash always runs internally in bf16 (per
# flash-attn's requirement), so fp32+flash means "bf16 attention math, fp32 outside
# attention" — not a clean fp32 baseline. Useful as an upper bound for flash speed
# on this hardware and as a "what does flash do for fp32 callers" reference.
#
# Different exp_name suffix and W&B group (-flash) so it doesn't collide with the
# scatter-backend run.
#
# Usage:
#   sbatch --job-name=PT2-sig4-wd1e4-fp32-flash-hipster scripts/run_pt2_long_sig4_wd1e4_fp32_flash_hipster.sh
...
EXP_NAME="pt2-sig4-wd1e4-fp32-flash-hipster${EXP_SUFFIX}"
