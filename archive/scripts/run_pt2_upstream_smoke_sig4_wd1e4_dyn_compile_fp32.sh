...
# 1-hour smoke: fp32 + dynamo-tuned compile.
# Tests whether the 4 dynamo knobs (cache_size_limit=256, force_parameter_static_shapes=False,
# capture_scalar_outputs=True, torch.compile(dynamic=True)) close the recompile thrash that
# the previous diagnostic (job 22569081) revealed.
# Recipe identical to the cancelled fp32 baseline 22430304, only difference: env knobs.
#
# Usage:
#   sbatch --job-name=PT2-sig4-wd1e4-dyn-compile-fp32 scripts/run_pt2_upstream_smoke_sig4_wd1e4_dyn_compile_fp32.sh
...
# Dynamo tuning knobs (picked up by train_omol.py main() and omol_module.setup()):
#   PSL_DYNAMO_TUNE=1 -> cache_size_limit=256, force_parameter_static_shapes=False,
#                       capture_scalar_outputs=True
#   PSL_COMPILE_DYNAMIC=1 -> torch.compile(net, dynamic=True)
export PSL_DYNAMO_TUNE=1
export PSL_COMPILE_DYNAMIC=1

# Keep recompile/graph_break logging on so we can confirm the knobs worked.
export TORCH_LOGS=recompiles,graph_breaks

cd /scratch-shared/ebekkers/platonic-omol/training

EXP_NAME="pt2-upstream-add-sig4-wd1e4-dyn-compile-fp32"

echo "=== SMOKE (1h, dyn-batching, sigma=4, wd=1e-4, fp32 + dynamo-tuned compile): ${EXP_NAME} ==="
echo "Date:  $(date)"
echo "Job:   ${SLURM_JOB_ID:-?} on $(hostname)"
echo "GPU:   $(nvidia-smi --query-gpu=name --format=csv,noheader)"
echo "Knobs: PSL_DYNAMO_TUNE=${PSL_DYNAMO_TUNE} PSL_COMPILE_DYNAMIC=${PSL_COMPILE_DYNAMIC}"
