#!/bin/bash
#SBATCH --job-name=smoke-pt2-upstream
...
#SBATCH --mem=120G
#SBATCH --output=logs/smoke-pt2-upstream-%j.out

# Smoke test for the upstream-port-pt2 branch:
# - 200 steps on train_4M
# - PT-2 settings (hidden_dim=1728, L=12, etc.)
# - Two configs in sequence: chgspin_mode=add, then chgspin_mode=concat
# - attention_backend=scatter (flash to be enabled once the build job lands)
#
# Goal: verify forward+backward, charge/spin grads nonzero, loss decreasing,
# no recompile-limit warnings, no shape errors from the wrapper changes.
...
echo "=== Smoke test: upstream-port-pt2 branch ==="
echo "Date: $(date)"
