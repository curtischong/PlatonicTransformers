...
# Hipster + flash-attn variant. Same recipe as run_pt2_long_sig4_wd1e4_bf16_hipster.sh
# but attention_backend=flash. Flash 2.8.3 is installed on the venv and supports
# sm_89 (the older "doesn't build for sm_89" comment was outdated). Flash should be
# faster than scatter and avoids the scatter-softmax bf16 precision hazard entirely
# (flash has internal fp32 softmax accumulators).
#
# Different exp_name suffix and W&B group (-flash) so it doesn't collide with the
# scatter-backend run.
#
# Usage:
#   sbatch --job-name=PT2-sig4-wd1e4-bf16-flash-hipster scripts/run_pt2_long_sig4_wd1e4_bf16_flash_hipster.sh
...
EXP_NAME="pt2-sig4-wd1e4-bf16-flash-hipster${EXP_SUFFIX}"
...
OVERRIDES=(
    ...
    wandb.group=pt2-sig4-wd1e4-precision-comparison-flash-hipster
    seed=1
)
